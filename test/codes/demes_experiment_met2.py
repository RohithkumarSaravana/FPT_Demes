...
Config naming: demes_<n_demes>_mr<migration_rate_pct>
  e.g.  demes_10_mr5   → n_demes=10, migration_rate=0.05
        demes_20_mr15  → n_demes=20, migration_rate=0.15

n_demes        : 5, 10, 15, 20
migration_rate : 0.02, 0.05, 0.10, 0.15

Usage (run from inside the met2/ folder):
    python codes/demes_experiment_met2.py <problem> <n_demes> <mr> [run_start] [n_runs]

    <mr> is the migration rate as a percentage integer: 2, 5, 10, 15
...
# ── CLI ───────────────────────────────────────────────────────────────────────
if len(sys.argv) < 4:
    print("Usage: python demes_experiment_met2.py <problem> <n_demes> <mr_pct> [run_start] [n_runs]")
    print("  mr_pct examples: 2, 5, 10, 15")
    sys.exit(1)

problem       = sys.argv[1]
N_DEMES       = int(sys.argv[2])
MR_PCT        = int(sys.argv[3])        # e.g. 5  means 5%
MIGRATION_RATE = MR_PCT / 100.0         # convert to fraction
...
if MR_PCT not in VALID_MR:
    raise ValueError(f"migration_rate must be one of {VALID_MR}%, got {MR_PCT}%")

# Config label used in folder name: e.g.  demes_10_mr5
config_label = f"demes_{N_DEMES}_mr{MR_PCT}"
print(f"Config: {config_label}  |  n_demes={N_DEMES}, migration_rate={MIGRATION_RATE} ({MR_PCT}%)")
