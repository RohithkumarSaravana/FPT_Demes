"""
functions.py — Shared helpers for the GP pipeline.

Two groups of functions:
  1. NumPy matrix utilities used by the lexicase-selection variants in
     selection.py to manipulate the (individuals x fitness cases) matrix
     during filtering (shuffling, removing rows/columns, aggregating rows
     into batches, comparing columns, etc.).
  2. The fuzzy-logic / aggregation primitives (WA, OWA, min/max, dilation,
     concentration, complement, fuzzy AND/OR) used as GP primitive-set
     operators when evolving fuzzy rule-based classifiers.
"""
...
def replace_substrings(input_string, replacements):
    """
    Replace every 'IN<i>' placeholder in input_string with
    replacements[i] (e.g. "IN0 + IN1" with replacements=["x","y"]
    becomes "x + y"). Used to substitute feature-column references into
    a GP individual's string representation before evaluation.

    Parameters:
        input_string -- string containing 'IN0', 'IN1', ... placeholders.
        replacements  -- list/sequence indexed by the number in each
                         placeholder.
...
def median_abs_deviation(arr, axis=0):
    ...
    the array's median. Used as a robust spread measure.

    This is a local copy of the same helper defined in selection.py —
...
def count_zeros_except_first_row(array):
    """Count zero entries in array, excluding its first row (which holds
    individual indexes rather than fitness data in the matrices this is
    used on)."""
...
def count_zeros(array):
    """Count zero entries across the whole array."""
    # Count the number of zeros
...
def WA(a, b, x):
    """Weighted average of a and b, with weight x (string, converted to
    float) applied to a and (1-x) applied to b. GP primitive-set operator."""
    x = float(x)
    return x*a+(1-x)*b

def OWA(a, b, x):
    """Ordered weighted average of a and b: weight x (string, converted
    to float) applied to the elementwise maximum and (1-x) to the
    elementwise minimum. GP primitive-set operator."""
    x = float(x)
    return x*np.maximum(a, b)+(1-x)*np.minimum(a, b)

def minimum(a, b):
    """Elementwise minimum of a and b. GP primitive-set operator."""
    return np.minimum(a, b)

def maximum(a, b):
    """Elementwise maximum of a and b. GP primitive-set operator."""
    return np.maximum(a, b)

def dilator(b):
    """Fuzzy dilation (square root): raises membership towards 1, used to
    weaken a fuzzy set's boundary. GP primitive-set operator."""
    return b**0.5

def dilator3(b):
    """Fuzzy dilation using a cube root (weaker effect than dilator)."""
    return b**(1/3)

def dilator4(b):
    """Fuzzy dilation using a fourth root (weaker effect than dilator3)."""
    return b**0.25

def concentrator(b):
    """Fuzzy concentration (square): lowers membership towards 0, used to
    sharpen a fuzzy set's boundary. GP primitive-set operator."""
    return b**2

def concentrator3(b):
    """Fuzzy concentration using a cube (stronger effect than concentrator)."""
    return b**3

def concentrator4(b):
    """Fuzzy concentration using a fourth power (stronger effect than concentrator3)."""
    return b**4

def fuzzy_AND(a, b):
    """Fuzzy AND (algebraic product) of a and b."""
    return a * b

def fuzzy_OR(a, b):
    """Fuzzy OR (algebraic sum) of a and b."""
    return a + b - a*b

def complement(b):
    """Fuzzy complement (negation) of membership value b."""
    return 1 - b