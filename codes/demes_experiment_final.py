"""
demes_experiment_final.py  —  Island-model GP, 5 datasets.

Config grid:
  n_demes        : 5, 10, 15
  migration_rate : 2%, 5%, 10%, 15%
  frequency      : 1, 5, 10, 25

Folder naming: demes_<n>_mr<pct>_freq<f>

Usage (run from inside the project root folder):
    python codes/demes_experiment_final.py <problem> <n_demes> <mr_pct> <freq> [run] [n_runs]
    python codes/demes_experiment_final.py wine 10  5  1 1 30
    python codes/demes_experiment_final.py wine 10  5 25 1 30
"""
...
# Ensure sibling modules (functions.py, fuzzify.py, algorithms_gp_final.py) are importable regardless of the working directory
...
# ── CLI ───────────────────────────────────────────────────────────────────────
if len(sys.argv) < 5:
    print("Usage: python codes/demes_experiment_final.py "
          "<problem> <n_demes> <mr_pct> <freq> [run_start] [n_runs]")
    print("  n_demes : 5, 10, 15")
    print("  mr_pct  : 2, 5, 10, 15")
    print("  freq    : 1, 5, 10, 25")
    sys.exit(1)
...
run            = int(sys.argv[5]) if len(sys.argv) > 5 else 1
N_RUNS         = int(sys.argv[6]) if len(sys.argv) > 6 else 30

VALID_DEMES = [5, 10, 15]
VALID_MR    = [2, 5, 10, 15]
VALID_FREQ  = [1, 5, 10, 25]

if N_DEMES not in VALID_DEMES:
    raise ValueError(f"n_demes must be one of {VALID_DEMES}, got {N_DEMES}")
if MR_PCT not in VALID_MR:
    raise ValueError(f"mr_pct must be one of {VALID_MR}, got {MR_PCT}")
if FREQ not in VALID_FREQ:
    raise ValueError(f"freq must be one of {VALID_FREQ}, got {FREQ}")

deme_size  = 500 // N_DEMES
n_migrants = max(1, round(deme_size * MIGRATION_RATE))
print(f"Config: n_demes={N_DEMES}, mr={MR_PCT}%, freq={FREQ}")
print(f"  deme_size={deme_size}, n_migrants={n_migrants}")

config_label = f"demes_{N_DEMES}_mr{MR_PCT}_freq{FREQ}"
...
for i in range(N_RUNS):
    ...
    population, logbook = algorithms.eaSimple_demes(
        ...
        frequency_parameter=FREQ
    )

    address = f"results_final/GP/{problem}/{config_label}/"
