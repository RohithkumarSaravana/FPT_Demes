...
def matrixDomain(DataFrame, numeric_columns=[], categorical_columns=[]):
    """
    Compute the value domain of every feature in DataFrame, for use by
    fuzzifyDataFrame when building fuzzy membership functions.
    ...
    Parameters:
        DataFrame            -- input data (rows with missing values are
                                dropped before computing domains).
        numeric_columns      -- names of columns to treat as numeric
                                (domain = [min, max]).
        categorical_columns  -- names of columns to treat as categorical
                                (domain = list of distinct category values).
                                Every column in DataFrame must appear in
                                exactly one of these two lists.

    Returns:
        List of per-feature domain descriptors, in the order of
        DataFrame's columns, as described above.
    """
...
def fuzzifyDataFrame(DataFrame, nSets, matrixDomain):
    """
    Convert a raw feature DataFrame into a fuzzified representation: each
    numeric feature is split into `nSets` overlapping triangular fuzzy
    sets spanning its domain (from matrixDomain), and each row's
    membership degree in every set becomes an output column; each
    categorical feature becomes one one-hot-style membership column per
    category (or a single 0/1 column when it has exactly two categories).
    Rows with missing values are dropped, consistent with matrixDomain.

    Parameters:
        DataFrame    -- input data to fuzzify (same columns/order used to
                        build matrixDomain).
        nSets        -- either an int (same number of fuzzy sets for every
                        numeric feature; categorical features always use
                        one set per category) or an array with one entry
                        per feature.
        matrixDomain -- per-feature domain descriptors as returned by
                        matrixDomain().

    Returns:
        DataFrame of fuzzy membership values, one or more columns per
        original feature, with the same (valid) row index as the input.
    """
    df = DataFrame.copy()
